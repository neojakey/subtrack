...
class UIHelper
{
    ...
    public static function SubscriptionCard(array $sub, array $category): string
    {
        ...
        // Pre-compute icon HTML for use in heredoc (ternaries and method calls not allowed inside heredoc)
        $iconEye = self::icon('eye', 'w-4 h-4');
        $iconPencil = self::icon('pencil', 'w-4 h-4');
        $iconTrash = self::icon('trash', 'w-4 h-4');
        ...
        return <<<HTML
        <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 p-5 hover:shadow-md transition-all duration-200 {$urgency}" data-sub-id="{$subId}">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-1">
                            ...
                            {$pauseBtn}
                            <button onclick="confirmDelete({$subId}, '{$name}')" class="btn-icon text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20" title="Delete">
                                {$iconTrash}
                            </button>
                        </div>
                    </div>
    ...
    public static function CalendarGrid(array $eventsByDay, int $year, int $month): string
    {
        ...
        for ($d = 1; $d <= $daysInMonth; $d++) {
            ...
            if ($isToday)
                $dayClass .= ' bg-blue-50 dark:bg-blue-900/10';
            elseif ($isPast)
                $dayClass .= ' bg-slate-50/60 dark:bg-slate-900/20 opacity-60';
